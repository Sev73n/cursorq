...
E["apps/tauri/package.json<br/>构建与运行脚本"] --> A
```
...
### 选择器命名规范与样式隔离
- 设计要点
  - 采用扁平化命名，避免深层嵌套；通过语义化类名表达组件角色（如 pill、panel、metric）。
...
## 结论
CursorQ 的样式系统以变量驱动为核心，配合清晰的类名约定与最小化动画策略，在 WebView 环境下实现了稳定、可维护且高性能的界面表现。通过等级映射与动效模块的解耦，系统具备良好的扩展性与可演进性。
