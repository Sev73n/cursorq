...
- apps/tauri/src/：前端主程序与吉祥物动图加载逻辑（main.ts、mascot-gifs.ts）
...
CJ["content/copy/jokes.json"]
CS["content/copy/states.json"]
MG["content/mascot/gifs/*.gif"]
end
subgraph "开发源"
AJ["assets/copy/jokes.json"]
AS["assets/copy/states.json"]
end
subgraph "远程配置"
...
subgraph "前端"
FM["apps/tauri/src/main.ts"]
MGIF["apps/tauri/src/mascot-gifs.ts"]
end
subgraph "核心库"
...
- 内容合并与远程同步：Rust 模块负责拉取远程 manifest.json 与文件，按“只追加、不覆盖”策略合并 jokes.json 与 states.json；二进制资源（动图）仅在本地不存在时写入。
- 内容选择与渲染：核心库根据用量状态选择文案池，进行宽度校验与循环选择；前端接收渲染数据并展示。
- 应用状态持久化：应用状态（含 jokeIndex 等）在 data 目录持久化，保证跨重启一致性。
- 吉祥物动图：前端通过 Tauri 命令加载内置动图，支持轮播与手动切换，内容更新后重载。
...
- 合并策略：
  ...
  - 二进制资源（动图等）：仅在本地不存在时写入，避免覆盖用户手动放入的文件。
- 远程配置：
  - enabled：是否启用远程同步。
...
- 前端主程序依赖核心库进行文案选择与渲染；依赖吉祥物模块进行动图展示与更新。
- 核心库依赖类型定义与应用状态持久化模块。
...
SYRS --> JOKES["jokes.json<br/>文案"]
SYRS --> STATES["states.json<br/>文案"]
SYRS --> GIFS["mascot gifs<br/>二进制资源"]
CORE["copy.ts<br/>文案选择/校验"] --> MAIN["main.ts<br/>渲染"]
...
### 内容文件组织结构与命名规范
- 目录
  - content/copy/：文案文件 jokes.json、states.json
  - content/mascot/：默认图 default.png/default.svg 与 gifs/ 动图目录
- 命名规范
